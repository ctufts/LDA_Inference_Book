...
```{r 2coins, echo = TRUE, warning=FALSE, message=FALSE, fig.cap = 'Bias of Two Coins: Theta 1'}
a = 2
b = 2
...
ggplot(ds, aes(x=theta1)) + geom_histogram(aes(y=..density..),color='#1303a5', fill='#200cd1') + 
  labs(title = expression(theta[1]~Estimate), x=expression(theta[1])) + geom_vline(xintercept = mean(ds$theta1), color='#b7091a')
```
```{r 2coins2, echo = TRUE, warning=FALSE,message=FALSE, fig.cap = 'Bias of Two Coins - Theta 2'}
ggplot(ds, aes(x=theta2)) + geom_histogram(aes(y=..density..),color='#1303a5', fill='#200cd1') + 
  labs(title = expression(theta[2]~Estimate), x=expression(theta[2])) + geom_vline(xintercept = mean(ds$theta2), color='#b7091a')

```

### Change Point Example
What if the problem we are solving is more complicated. Let's say I flip a coin repeatedly, but at some point I switch to another coin with a different bias ($/theta$). I want to detect the point in time when coin 1 was swapped out for coin 2. 
