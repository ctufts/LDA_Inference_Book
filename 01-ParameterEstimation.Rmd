...
You will often see Bernoulli distribution mentioned as a special case of the Binomial distribution. The difference between the two is that a Bernoulli distribution only looks at the outcome of a single experiment, not a string of experiments. 

Add citation for [@kerns2010introduction]
------------Image Placeholder-----------------------
...
$$
f_{x}(x)=\theta^{x}(1-\theta)^{1-x}, \hspace{1cm} x = \{0,1\} 
$$
...
$$ 
\underbrace{p(\Theta|D)}_{posterior} = {\overbrace{\bf \Large p(D|\Theta)}^{\bf \Large LIKELIHOOD}
  \overbrace{p(\Theta)}^{prior} \over \underbrace{p(D)}_{evidence}}
$$

So what does this actually 
mean? In the case of a bernoulli distribution it is fairly straight forward. The 
value of $\theta$ that maximizes the likelihood is the number of heads over the 
number of flips. 

Maximum Likelihood
$$
L = log \prod\limits_{n=1}^N \theta^{x}(1-\theta)^{1-x} \\
L = \sum\limits_{n=1}^N \theta^{x}(1-\theta)^{1-x} \\
L = n^{(1)}log(\theta) + n^{(0)}log(1-\theta)
$$
Differentiate with respect to $\theta$:

$$
{dL \over d\theta} =  {n^{(1)}\over \theta} - {n^{(0)}\over 1-\theta}
$$
...
```{r bernoulliml, echo = TRUE, fig.cap = 'Bernoulli Maximum Likelihood'}
library(ggplot2)
heads = 1:10
flips = 10
...
ggplot(ds, aes(x = heads,
  ...
  labs(y='\U0398', x="Number of Heads", title ="ML Parameter Estimation: 10 Bernoulli Trials") +
