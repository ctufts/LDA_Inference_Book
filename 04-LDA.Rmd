...
```{r ldaGenerativeAllStatic, echo=TRUE, warning=FALSE, message=FALSE, cache=TRUE} 
...
```{r ldaGenerativeVariedLength, cache=TRUE} 
k <- 2 # number of topics
M <- 10 # let's create 10 documents
vocab <- c('\U1F4D8', '\U1F4D5', '\U1F4D7')
alphas <- rep(1,k) # topic document dirichlet parameters
...
```{r ldaGenerativeVaryTDLength, echo = TRUE, warning=FALSE, message=FALSE, cache = TRUE}
