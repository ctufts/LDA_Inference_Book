...
```{block2, heads-definition, type='rmdnote'}   
...
```{block2, beta-gamma, type='rmdnote'}
...
```{block2, weak-naive, type='rmdnote'}
The terms weak, uninformed, and naive are often used interchangeably when referencing priors. The same goes for the terms strong and informed. 
```
...
```{r MarkovChain, echo = FALSE, fig.cap = 'Markov Chain: Rain or Shine', out.width = '50%', out.height='50%', fig.align='center'}
knitr::include_graphics("Images/MarkovChain.png")
```
...
```{r MonteCarlo, echo = FALSE, fig.cap = 'Monte Carlo: Estimating &#960;', out.width = '75%', out.height='75%', fig.align='center'}
knitr::include_graphics("Images/MonteCarlo.png")
```
...
The process without the math is shown in Figure \@ref(fig:GibbsViz). The red circles represent the parameters yet to be estimated in this iteration where the blue represent those that have been previously estimated during the current iteration. Note that the purple circle in each row is the parameter currently being estimated in that step (the current row) and that it takes into account all the available info, i.e. all the red and blue circles in that row.     

```{r GibbsViz, echo = FALSE, fig.cap = 'Gibbs Sampling - The Process', out.width = '75%', out.height='75%', fig.align='center'}
knitr::include_graphics("Images/GibbsSamplingViz.png")
```




The calculations used for our example are shown in Equation \@ref(eq:TwoCoinsGibbsGeneral). Note that since the two coins are independent of one another, we are sampling from a Beta distribution based on each coin's priors and evidence then repeating.   
