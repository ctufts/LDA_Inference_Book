...
Note that the evidence term is actually a constant value since it is an integral over all values of $\theta$. Since it is a constant we can drop it from our equation. 

$$
\begin{aligned}
p(\theta|z, N) &= \theta^{z}(1-\theta)^{(N-z)}{{\theta^{(a-1)}(1-\theta)^{(b-1)}}\over \beta(a,b)} \\\\
