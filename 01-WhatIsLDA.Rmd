...
So let's move on to generating a specific location. We know that different locations will vary in terms of which habitats are present. For example, a beach contains land, sea, and air, but some areas inland may only contain air and land, for instance a desert.  We can define the mixture of these types of habitats in each location. For example, an example beach is 1/3 land, 1/3 sea, and 1/3 air. We can think of these more specific locations as documents. To review: a given location (document) contains a mixture of land, air, and sea (topics) and each of those contain different mixtures of animals (words).  
...
phi_ds %>% kable() %>% kable_styling(bootstrap_options = "striped",
                full_width = F)
```


And so if we want to generate a document based off this we would use those probabilities in a straightforward manner:
...
Ok, so we saw that we can generate these collections of animals that are representative of the given location. What if we have thousands of locations and we want to know the mixture of land, air, and sea that are present? And what if we had no idea where each animal spends its time? LDA allows us to infer both of these peices of information.   Similar to the locations (documents) I generated above, I will create 1000 random documents with varying length. 
...
(head(round(theta_samples,2))) %>% kable() 
```
...
model. [@blei2003latent]




Some other information you should keep in mind:
1. A word can part of multiple different topics.  In the case of a penguin, it would belong to the sea and land topics, but would have a probability of zero in an air topic. 
2. 