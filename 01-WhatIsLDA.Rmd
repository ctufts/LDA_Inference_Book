...
The estimated topic word distributions are shown in Table 1.4 and can be compared to the true distributions in Table 1.5.
...
kable(round(phi_est, 2),col.names = estimated_topic_names ,caption = 'Estimated word distribution for each topic')
```

```{r echo=FALSE}
kable(t(round(phi,2))[,estimated_topic_names],caption = 'The word distribution for each topic used to build the documents')
```
...
```{r echo = FALSE}
(head(round(theta_samples,2)[,estimated_topic_names])) %>% kable(caption='The Real Topic Distributions for the First 5 Documents') 
```
