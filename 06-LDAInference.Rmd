...
* **Vocabulary:** The unique list of words across all documents. This means our document word matrix has one row for each document and one column for each word in our vocabulary. 
* **Hyperparemeters:**
...
$$
\begin{equation}
\phi^{j}_{i} = { C^{WT}_{ij}  + \beta \over  \sum_{k=1}^{W} C^{WT}_{kj} + W \beta}
(\#eq:phiEstimate)
\end{equation}
$$
...
Equations \@ref(eq:thetaEstimation) and \@ref(eq:phiEstimate) are from [@steyvers2007probabilistic]
...
As in previous chapters,  Gibbs sampling is used to infer the value of each $\theta$ we calculate the value of of 
$p(\theta_{}|)$
